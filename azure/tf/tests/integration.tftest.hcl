# File-level variables can only reference other global variables, not run block outputs.
variables {
...
run "classic_cluster_spoke" {
  ...
  variables {
    databricks_host = run.test_initializer.outputs.outputs.spoke_workspace_info["prod"].workspace_url
    tags            = { SRA = var.sra_tag }
  }
}
...
run "bundle_deploy" {
  ...
  variables {
    ...
    environment = {
      ...
      BUNDLE_VAR_catalog_name  = run.test_initializer.outputs.outputs.spoke_workspace_catalog["prod"]
