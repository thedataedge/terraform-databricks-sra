...
module "hub" {
  ...
  virtual_network_peerings = var.create_workspace_vnet && length(var.spokes) > 0 ? { for k in keys(var.spokes) : "spoke_${k}" => { remote_virtual_network_id = module.spoke_network[k].vnet_id } } : {}
  ...
  is_firewall_enabled      = var.is_firewall_enabled
